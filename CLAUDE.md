...
- **`ResourceEngine`** — owns the decision graph (lazily initialized, `volatile` field). Catches `MalformedBodyException` → 400; all other exceptions are stored on `context.setException()` then routed through the `HANDLE_EXCEPTION` handler node (default 500), so resource classes can override `@Decision(HANDLE_EXCEPTION)` to customize error responses.
